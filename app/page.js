      ...
      <section className="w-[37%] flex-col gap-[5.125rem] self-center items-center max-h-[25rem] flex">
        <div className="flex-col gap-[2.25rem] items-center flex">
          <div className="text-center">
            <h2 className="text-[#34333e] text-[2.25rem] font-semibold leading-[2.937rem] tracking-[0.0312rem]">Provide your phone number
            </h2>
            <p className="text-[#585858] text-[0.9375rem] font-light leading-[1.375rem]">Kindly use an active WhatsApp number, and we'll send a  confirmation message to the provided number.
            </p>
          </div>
          <InputField />
        </div>
        ...
        <div className='flex-col gap-[1.12rem] w-full flex'>
          ...
          <MyButton text={'Have an Account? Login'} color={'#ffffff'} border={' border_color'} mode={'dark'} />
        </div>
      </section>
    </main>
  )
  ...
  return (
    <div className="gap-[0.3125rem] w-full items-center flex">
        ...
        <span className=' gap-[0.31rem] h-[2.9375rem] py-[0.3125rem] pr-[0.875rem] pl-[0.6875rem] items-center flex'>
          ...
          <span>
            <svg xmlns="http://www.w3.org/2000/svg" width="11" height="11" viewBox="0 0 11 11" fill="none">
              <path d="M8.53125 4.1875L5.25 7.46875L1.96875 4.1875" stroke="#34333E" stroke-width="0.65625" stroke-linecap="round" stroke-linejoin="round" />
            </svg>
          </span>
        </span>
      )} >
      ...
      <label className='relative h-[2.9375rem] py-[0.3125rem] px-[0.875rem] rounded-lg border border-[#34333e] shadow-sm items-end flex-1 flex '>
        <span className='absolute top-0 left-3.5  text-[#999] text-[0.625rem] leading-5'>Phone Number</span>
...
export const MyButton = ({ text, color, border, mode }) => {
  return (
    <button className={`items-center justify-center py-[1.12rem] text-[1.125rem] px-[3.125rem] h-[3.375rem] rounded-[0.25rem]  bg-[${color}] flex ${border} `}>
      <span className={`tracking-[-0.01875rem] leading-[1.875rem] ${mode}`}>{text}</span>
    </button>
  )
}